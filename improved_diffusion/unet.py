...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        self.rrdb = RRDBNet(in_nc=condition_input_channel, nb=rrdb_blocks, out_nc=model_channels)
    ...
    def forward(self, x, timesteps, number_of_annotators=None, conditioned_image=None, random_p=None, inference=False, first_time_step=100):
        ...
        :return: an [N x C x ...] Tensor of outputs.
        """
        # assert (number_of_annotators is not None) == (
        #     self.total_num_of_annotators is not None
        ...
        if inference:
            if timesteps[0] == (first_time_step - 1):
                self.cache_rrdb_result = self.rrdb(conditioned_image.type(self.inner_dtype))
    ...
    def get_feature_vectors(self, x, timesteps, y=None):
        ...
        hs = []
        emb = self.time_embed(timestep_embedding(timesteps, self.model_channels))
        if self.total_num_of_annotators is not None:
