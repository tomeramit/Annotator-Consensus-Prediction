...
class TrainLoop:
    ...
    def run_loop(self, max_iter=250000, start_print_iter=100000, number_of_generated_instances=9):
        if dist.get_rank() == 0:
            pbar = tqdm()
        while (
            self.step < max_iter
            ...
            if self.step % (self.save_interval * 4) == 0:
            ...
            if self.step % self.save_interval == 0 and self.step >= start_print_iter or self.step == 60000:
                if self.run_without_test:
                    ...
                    ema_val_miou = sampling_major_vote_func(self.diffusion, self.ema_model, output_folder=output_folder,
                                                            ...
                                                            clip_denoised=self.clip_denoised, step=self.step, number_of_generated_instances=number_of_generated_instances)
    ...
    def save_state_dict(self):
        ...
        if dist.get_rank() == 0:
            ...
            with bf.BlobFile(bf.join(get_blob_logdir(), f"model_{self.step}.pt"), "wb") as f:
            ...
            self.current_model_checkpoint_name = bf.join(get_blob_logdir(), f"model_{self.step}.pt")
