...
from datasets.prostate import Prostate1Dataset
...
def main():
    ...
    exp_name = f"prostate_1_floating_p_{args.image_size}_{args.rrdb_blocks}_{args.lr}_{args.batch_size}_{args.diffusion_steps}_{str(args.dropout)}_{args.weight_decay}_{args.n_gen}_{MPI.COMM_WORLD.Get_rank()}"
    ...
    if args.soft_label_training:
        ...
        args.number_of_annotators = Prostate1Dataset.get_number_of_annotators()
    ...
    data = load_data(
        dataset_class=Prostate1Dataset,
    ...
    prostate_dataset = Prostate1Dataset(
        mode='val',
        image_size=args.image_size,
    ...
    logger.log(f"gpu {MPI.COMM_WORLD.Get_rank()} / {MPI.COMM_WORLD.Get_size()} validation images {len(prostate_dataset)}")
